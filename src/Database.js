...
function Database({ columns, data }) {
    ...
    return (
        <div>
            <label>
                Rows Displayed:
                <select name="rowsDisplayed" value={numRows} onChange={(e) => setNumRows(e.target.value)}>
            ...
            <table>
                <thead>
                    <tr>
                        {columns.map((col) => (
                            <th onClick={() => updateSortOrder(col, !order[col].ascending)} key={v4()}>{col.toUpperCase()}</th>
                        ))}
                    </tr>
                </thead>
