...
function MortgageCalculator(props) {
    ...
    const validateInput = () => {
        ...
        if(!isNumber(apr) || apr === '') {
            validationErrors.push("Please enter a valid annual percentage rate.");
        }
    ...
    const calculateResults = () => {
        ...
        const monthlyPayments = loanAmt * (0.0025 * (Math.pow((1.0025), 360))/(Math.pow((1 + 0.0025), 360)) - 1);
        console.log("n", n)
    ...
    return (
        <form>
            <label htmlFor="loan-amt">Loan Amount</label> <input type="text" name="loan-amt" value={loanAmt} onChange={(e) => {
                setLoanAmt(e.target.value);
            }} />
            <label htmlFor="apr">Annual percentage rate</label> <input name="apr" type="text" value={apr} onChange={(e) => {
                setAPR(e.target.value);
            }} />
            <label htmlFor="loan-term">Loan term: </label> <input type="text" name="loan-term" value={loanTerm} onChange={(e) => {
